...
void ImGui_ImplGlfwGL3_NewFrame()
{
	...
	if (glfwGetWindowAttrib(g_Window, GLFW_FOCUSED) && !Input::IsMouseGrabbed())
	{
...
{
	int old_size = s_log.Buf.size();
	va_list args;
	va_start(args, fmt);
	s_log.Buf.appendv(fmt, args);
	va_end(args);
	for (int new_size = s_log.Buf.size(); old_size < new_size; old_size++)
		if (s_log.Buf[old_size] == '\n')
			s_log.LineOffsets.push_back(old_size);
	s_log.ScrollToBottom = true;
}
